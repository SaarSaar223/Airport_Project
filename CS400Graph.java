...
public class CS400Graph<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType>, IDijkstra<NodeType,EdgeType> {
   ...
   /**
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
          ...
          Vertex target = extendBy.target;
          NodeType sourceVertex = copyPath.end.data;
          NodeType targetVertex = extendBy.target.data;
          if (containsEdge(sourceVertex, targetVertex)) {
            this.dataSequence.add(target.data);
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end){
        ...
        Vertex beginning = vertices.get(start);
        Vertex ending = vertices.get(end);
        Vertex currentNode = vertices.get(start);
        Path first = new Path(beginning);
        Path finalPath = null;
        pq.add(first);
        List<NodeType> visitedNodes = new LinkedList<NodeType>();
        
        if(start.equals(end)) {
          return new Path(beginning);
        }
        
        while(!currentNode.equals(ending)) {
          if(pq.isEmpty()) {
            break;
          }
          ...
          for (int i = 0; i < currentNode.edgesLeaving.size(); ++i) {
            Path newPath = new Path(currentPath, currentNode.edgesLeaving.get(i));
            pq.add(newPath);
          }
        }
    ...
    @Override
    public CS400Graph<NodeType, EdgeType> getMinSpanningTree(NodeType start) throws NoSuchElementException{
      PriorityQueue<Path> pq = new PriorityQueue<Path>();
      if(!vertices.containsKey(start)) {
        throw new NoSuchElementException();
      }
      CS400Graph<NodeType, EdgeType> MSTGraph = new CS400Graph<NodeType, EdgeType>();
      Vertex currentNode = vertices.get(start);
      MSTGraph.insertVertex(start);
      for(int i = 0; i < currentNode.edgesLeaving.size(); ++i) {
        Path newPath = new Path(currentNode);
        Path minEdge = new Path(newPath, currentNode.edgesLeaving.get(i));
        pq.add(minEdge);
      }
      
      while(!pq.isEmpty()) {
        if(vertices.size() == MSTGraph.vertices.size()) {
          return MSTGraph;
        }
        Path currentPath = pq.remove();
        currentNode = currentPath.end;
        if(MSTGraph.containsVertex(currentNode.data)) {
          continue;
        }
        else {
          MSTGraph.insertVertex(currentNode.data);
          Edge temp = null;
          for(int i = 0; i < currentPath.start.edgesLeaving.size(); ++i) {
            if(currentPath.start.edgesLeaving.get(i).target.equals(currentNode)) {
              temp = currentPath.start.edgesLeaving.get(i);
            }
          }
          
          MSTGraph.insertEdge(currentPath.start.data, currentPath.end.data, temp.weight);
        }
        for(int i = 0; i < currentNode.edgesLeaving.size(); ++i) {
          Path newPath = new Path(currentNode);
          Path minEdge = new Path(newPath, currentNode.edgesLeaving.get(i));
      ...
      throw new NoSuchElementException("A MST Cannot Be Made With the Given Nodes!");
    }

    @Override
    public double getMinSpanningTreeCost(NodeType start) {
      CS400Graph<NodeType, EdgeType> MSTGraph = getMinSpanningTree(start);
      double totalWeight = 0;
      Enumeration enu = MSTGraph.vertices.keys();
      while (enu.hasMoreElements()) {
        NodeType temp = (NodeType) enu.nextElement();
        if(MSTGraph.vertices.get(temp).edgesLeaving == null) {
          continue;
        }
        for (int j = 0; j < MSTGraph.vertices.get(temp).edgesLeaving.size(); ++j) {
          totalWeight += MSTGraph.vertices.get(temp).edgesLeaving.get(j).weight.doubleValue();
        }
      }
      return totalWeight;
    }    
