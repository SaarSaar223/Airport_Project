...
public class CS400GraphFront<NodeType, EdgeType extends Number> implements GraphADT<NodeType, EdgeType> {
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        if (!vertices.containsKey(start) || !vertices.containsKey(end)) {
            throw new NoSuchElementException("Start or End may not in the vertices");
        }
        ...
        // Shortest path from startVertex to each vertex
        LinkedList<Vertex> frontier = new LinkedList<>();
        paths.put(startVertex, shortestPath);
        frontier.add(startVertex);

        // Finding the shortest path
        while (frontier.size() != 0) {
            // Getting edges for the first element of frontier
            LinkedList<Edge> edges = frontier.peek().edgesLeaving;
            for (Edge edge : edges) {

                // if this is a self-loop, jump to next iteration
                if (edge.target.equals(frontier.peek()))
                    continue;

                // if the target is in the path
                if (paths.containsKey(edge.target)) {

                    double currentDis =
                        paths.get(frontier.peek()).distance + edge.weight.doubleValue();
                    if (paths.get(edge.target).distance > currentDis)
                        paths.replace(edge.target, new Path(paths.get(frontier.peek()), edge));
                }
                // if the target is not in the path
                else {
                    paths.put(edge.target, new Path(paths.get(frontier.peek()), edge));
        ...
        if (!paths.containsKey(endVertex))
            throw new NoSuchElementException("No path can be found");
